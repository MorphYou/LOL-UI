...
def get_summoner_info(game_name, puuid, region='europe'):
    """Get summoner information directly by name"""
    try:
        urls = get_region_urls(region)
        # Try getting summoner directly by name
        url = f"{urls['platform']}/lol/summoner/v4/summoners/by-name/{game_name}"
        response = requests.get(url, headers=RIOT_HEADERS)
        
        if response.status_code == 200:
            summoner_data = response.json()
            print("Summoner data found:", summoner_data)  # Debug print
            return summoner_data
        else:
            print(f"Error getting summoner info: {response.status_code}")  # Debug print
...
def search_player():
    ...
    summoner_info = get_summoner_info(game_name, account_info['puuid'], region)
    if not summoner_info:
        return jsonify({'error': 'Summoner information not found'}), 404
    ...
    response_data = {
        ...
        'profileIconId': summoner_info.get('profileIconId', 1),
        'summonerLevel': summoner_info.get('summonerLevel', 0),
